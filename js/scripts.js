...
//=============== map  ==============
	...
	$('a.ajax').bind("click", function() {
		$(".video-container video , .resp-video video, .resp-video iframe").remove();
...
function initparallax() {
	...
	if(!trueMobile)
    {
        var v = $('<video autoplay loop muted  class="bgvid"><source src="video/promo2.mp4" type="video/mp4"></video>');
...
//============== custom menu ==============
function activemenu() {
    //TODO ЕСЛИ ПОДХОДЯЩАЯ СТРАНИЦА, ТО type=checkbox
    //.menu-level-1 input[type=checkbox]:checked + label + ul,
    //.menu-level-1 input[type=checkbox]:checked + label:nth-of-type(n) + ul {
    //        display: block;
    //    }
}
//=============== init ajax  ==============
$(function() {
    $.coretemp({
        reloadbox: "#wrapper",
		ctbTrigger: "a.ajax",
		loadErrorMessage: "Данной страниы не существует!", // 404 error text
        loadErrorBacklinkText: "Главная", // 404 back button  text
        outDuration: 750,
        inDuration: 700
    });
    readyFunctions();
...
$( document ).ready(function() {
    $("header").removeClass("sticky");
    var autoPaging = setInterval(function(){$('.customNavigation .next-slide').click();},10*1000);
    $('.testimonilas-holder p').click(function() {
        clearInterval(autoPaging);
    });
    $(".insta-widget").click(function () {
        window.location.replace("https://www.instagram.com/studebakers_studio/");
    });
    $("#market").click(function () {
        window.open('http://dmexperts.pro/', 'name');
    })
